...
if (ALL || ONLY_VER) {
  ...
  try {
    ...
    let svs = vj.sitecheck_js_size || 0;
    if (!svs) {
      // r7 version.json may omit the size key — measure the actual file instead
      try { svs = fs.statSync(path.join(PUBLIC,'assets','sv-check.js')).size; } catch(e) {}
    }
    if (svs >= 85000) ok(`sv-check.js size = ${svs}b (\u2265 85000)`);
    else fail('sv-check.js too small', `${svs}b, expected \u2265 85000 — may indicate stripped guards`);
    ...
    const dchPkgMatch = dchContent.match(/String\(v\.package_number\)==='([0-9A-Za-z]+)'/);
...
if (ALL) {
  ...
  const REQUIRED_LABELS = ['What we found', 'What this means', 'What still needs checking', 'Next useful step'];
  ...
  // r7: forbidden public wording must NOT appear as rendered headings/CTAs
  const FORBIDDEN_PUBLIC = [
    ['Advantages heading', 'signal-heading\">Advantages'],
    ['Disadvantages heading', 'signal-heading\">Disadvantages'],
    ['old CTA Find Out What My Land Can Do', 'Find Out What My Land Can Do'],
    ['old full-report CTA link', '/full-report.html'],
    ['free report unlocked wording', 'free report unlocked'],
    ['Register to continue gate', 'Register to continue'],
  ];
  for (const [label, needle] of FORBIDDEN_PUBLIC) {
    if (svContent.includes(needle)) fail(`r7: forbidden public wording present: ${label}`, needle);
    else ok(`r7: absent — ${label}`);
  }
  // r7: Professional Review is the public CTA target
  if (svContent.includes('/professional-review.html')) ok('r7: Professional Review CTA present');
  else fail('r7: Professional Review CTA missing', '/professional-review.html not found in sv-check.js');
