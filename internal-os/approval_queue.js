...
// Map a prep sheet + enquiry into the Approval Console queue.json schema.
function toQueueRecord(e, sheet, qtype, prepFile, emailFile, quoteFile){
  const base = sheet.status; // PASS / REVIEW / BLOCKED
  // console status derivation
  let status;
  if(base==='BLOCKED') status='NEEDS_INFO';
  else if(base==='REVIEW') status='WAITING_APPROVAL';
  else if(qtype) status='QUOTE_READY';   // PASS + a paid pathway
  else status='PREP_READY';              // PASS, no quote
  // risk flags derived from the sheet
  const flags=[];
  const nc = sheet.notConfirmed.join(' ').toLowerCase();
  if(/could not be confidently matched|address not matched/.test(nc)) flags.push('address_not_matched');
  if(/minimum lot size: not confirmed|not confirmed for this zone/.test(nc)) flags.push('min_lot_not_confirmed');
  if(/zone: not confirmed/.test(nc)) flags.push('zone_not_confirmed');
  if(sheet.verified.some(v=>/Heritage indicator: present/i.test(v))) flags.push('heritage_detected');
  if(sheet.verified.some(v=>/Flood planning indicator: present/i.test(v))) flags.push('flood_detected');
  if(sheet.verified.some(v=>/Bushfire prone indicator: present/i.test(v))) flags.push('bushfire_detected');
  if(sheet.userEntered.length) flags.push('user_entered_only');
  if(qtype) flags.push('quote_needs_price');
  // risk level
  let risk='normal';
  if(base==='BLOCKED') risk='urgent';
  else if(base==='REVIEW') risk='high';
  else if(qtype) risk='high';
  return {
    enquiry_id: e.submission_id,
    name: e.name||'', email: e.email||'', phone: e.phone||'',
    address: e.address||'', purpose: sheet.purpose,
    status, baseStatus: base, confidence: sheet.confidence,
    created_at: e.created_at || new Date().toISOString(),
    action: sheet.recommendedAction,
    pathway: (qtype || sheet.pathway[0] || 'Not enough information'),
    risk, flags,
    prep: prepFile, email_file: emailFile, quote: quoteFile||'',
    summary: sheet.summary,
    draft_excerpt: (sheet.verified.slice(0,2).map(v=>v.split('  [')[0]).join('; ')||'—'),
    quote_excerpt: qtype ? (qtype+'. Price: T to confirm.') : ''
  };
}
...
async function main(){
  ...
  for(const e of enquiries){
    ...
    // ---- Console-schema record for queue.json ----
    queueRecords.push(toQueueRecord(e, sheet, qtype, prepFile, emailFile, quoteFile));
    console.log(`[${sheet.status}/${sheet.confidence}] ${e.submission_id} ${e.name||'—'} | ${e.address||'(no address)'} | email:${tname}${qtype?' | quote:'+qtype:''}`);
  }

  // write queue.json (Approval Console schema) — the bridge between queue and console
  fs.writeFileSync(path.join(outDir,'queue.json'), JSON.stringify(queueRecords,null,2));
  console.log('[queue.json] '+path.join(outDir,'queue.json'));
...
module.exports = { chooseTemplate, quoteType, toQueueRecord };
